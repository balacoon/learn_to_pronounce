...
class AddonManager(object):
    ...
    def __init__(self, work_dir: str, locale: str):
        ...
        """
        self._path = os.path.join(work_dir, ADDON_FILE_NAME)
        if not os.path.isfile(self._path):
            # there is no addon in work dir, create a dummy one
            addon_dict = {pm.addon_field_to_string(pm.AddonFields.ID): pm.addon_identifier(),
            ...
            if addon_locale != locale:
                raise RuntimeError("Addon file exists and locale inside [{}] doesn't match one specified [{}].".format(addon_locale, locale))
    ...
    def add_lexicon(self, pd: PronunciationDictionary, graphemes: List[str], phonemes: List[str]):
        """
    ...
    def add_pronunciation_fst(self, fst_path: str):
        ...
        fst_path: str
            path to FST model
        """
        self._add_fst(pm.addon_field_to_string(pm.AddonFields.FST_PRONUNCIATION_GENERATOR), fst_path)
    ...
    def add_spelling_fst(self, fst_path: str):
        ...
        fst_path: str
            path to FST model
        """
        self._add_fst(pm.addon_field_to_string(pm.AddonFields.FST_SPELLING_GENERATOR), fst_path)
