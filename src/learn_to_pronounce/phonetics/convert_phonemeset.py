...
def parse_args():
    ap = argparse.ArgumentParser(
        description="Helper script that converts phonemeset in a lexicon"
    )
    ap.add_argument("in_path", help="Lexicon to convert")
    ap.add_argument("out_path", help="Path to put converted lexicon to")
    ap.add_argument(
        "--source", help="Source phonemeset", choices=["arpa"], default="arpa"
    )
    ap.add_argument(
        "--target",
        help="Target phonemeset",
        choices=["xsampa", "ipa"],
        default="xsampa",
    )
    args = ap.parse_args()
...
def main():
    ...
    with open(args.in_path, "r", encoding="utf-8") as ifp, open(
        ...
        for line in ifp:
            ...
            if args.target == "xsampa":
                ...
                raise RuntimeError(
                    "Unsupported target phonemeset: {}".format(args.target)
                )
